...
namespace MinigunViking
{
    class StartMenu
    {
        ...
        Boolean arrowKeyRegistered = false;
        ...
        public int Update(MouseState mouse, Point mpos, KeyboardState keys, GameTime gameTime, bool gamePaused)
        {
            ...
            {
                if (!arrowKeyRegistered)
                    highlight--;
                if (highlight < 1 && !arrowKeyRegistered && menustate == 0)
                    highlight = 3;
                if (highlight < 1 && !arrowKeyRegistered && menustate == 1)
                    highlight = 10;
                arrowKeyRegistered = true;
            }
            if (keys.IsKeyDown(Keys.Down))
            {
                if (!arrowKeyRegistered)
                    highlight++;
                if (((highlight > 3 && menustate == 0) || (highlight > 10 && menustate == 1)) && !arrowKeyRegistered)
                    highlight = 1;
                arrowKeyRegistered = true;
            }

            if (keys.IsKeyUp(Keys.Up) && keys.IsKeyUp(Keys.Down))
                arrowKeyRegistered = false;
